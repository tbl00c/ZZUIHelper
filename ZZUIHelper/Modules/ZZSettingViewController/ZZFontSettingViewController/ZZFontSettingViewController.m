...
#import "ZZFontSettingViewController.h"
#import "ZZSettingItemsAddViewController.h"

@interface ZZFontSettingViewController ()
...
@implementation ZZFontSettingViewController
...
- (void)loadData
{
    self.data = [ZZUIHelperConfig sharedInstance].fonts.mutableCopy;
...
- (void)tableView:(NSTableView *)tableView setObjectValue:(id)object forTableColumn:(NSTableColumn *)tableColumn row:(NSInteger)row
{
    if (row < self.data.count) {
        ...
        [ZZUIHelperConfig sharedInstance].fonts = self.data;
    }
}
...
- (IBAction)addLotButtonClick:(id)sender {
    ...
    [addVC setOkButtonClickAction:^(NSArray *data) {
        if (data.count > 0 && weakSelf) {
            ...
            [ZZUIHelperConfig sharedInstance].fonts = weakSelf.data;
...
- (IBAction)resetButtonClick:(id)sender {
    [self.view.window makeFirstResponder:self.view];
    [self.view.window makeFirstResponder:self.tableView];
    [[ZZUIHelperConfig sharedInstance] resetToDefaultFonts];
    [self loadData];
}
